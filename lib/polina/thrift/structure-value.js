...
// Если структура в структуре, как передать тип?
 ...
 */
polina.thrift.StructValue = function(valueInfo, isError) {
   ...
   * @type {string}
   */
  this.__type = valueInfo.getStructType();
  ...
  while (cursor.isParsed()) {
    if (this.__element === null) {
      ...
      if (field === null) {
        ...
        this.__element = polina.thrift.createValue(field.type, this.__valueInfo
            ,false);
      }
    }
