  ...
  this.__header = null;
	...
	this.__compliteMessage = function(){
		...
		if (self.__header.type === polina.thrift.MessageTypes.REPLY){
			polina.thrift.compliteReply(self.__valueQueue, complete, cancel);
		}

		if (self.__header.type === polina.thrift.MessageTypes.EXCEPTION){
			polina.thrift.compliteException(self.__valueQueue, cancel);
		}

		if (self.__header.type === polina.thrift.MessageTypes.CALL){
			polina.thrift.compliteCall(self.__valueQueue, self.__header.name);
		}
	}
