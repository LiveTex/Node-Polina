...
polina.thrift.PacketHandler.prototype.process = function(cursor, chunk) {
  ...
  while (!this.__isComplete && cursor.isParsed() && this.__header !== null) {
    ...
    if (this.__value === null) {
      var fieldHeader = this.__protocol.readFieldBegin(cursor, chunk);
      if (cursor.isParsed() && fieldHeader.type === polina.thrift.Types.STOP) {
      ...
      if (cursor.isParsed()) {
        console.log('Create  value with type ', fieldHeader.type);
