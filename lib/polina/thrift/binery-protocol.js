 ...
 * @inheritDoc
 */
polina.thrift.BineryProtocol.prototype.serializeValue = function(value, field) {
  var output = new Buffer('');

  if ((typeof value === 'string') &&
      field.getType() === polina.thrift.Types.STRING) {
    output = this.writeString(value);
  } else if (typeof value === 'boolean' &&
      field.getType() === polina.thrift.Types.BOOL) {
    output = this.writeBool(value);
  } else if (typeof value === 'number') {
    if (field.getType() === polina.thrift.Types.BYTE) {
      output = this.writeByte(value);
    } else if (field.getType() === polina.thrift.Types.I16) {
      output = this.writeI16(value);
    } else if (field.getType() === polina.thrift.Types.I32) {
      output = this.writeI32(value);
    } else if (field.getType() === polina.thrift.Types.I64) {
      output = this.writeI64(value);
    } else if (field.getType() === polina.thrift.Types.DOUBLE) {
      output = this.writeDouble(value);
    }
  } else if (typeof value === 'object') {
    if (field.getType() === polina.thrift.Types.MAP) {
      output = this.serializeMap(value, field.getKInfo(), field.getVInfo());
    } else if (field.getType() === polina.thrift.Types.SET ||
               field.getType() === polina.thrift.Types.LIST) {
      output = this.serializeArray(value, field.getVInfo());
    } else if (field.getType() === polina.thrift.Types.STRUCT) {
...
polina.thrift.BineryProtocol.prototype.serializeStruct = function(userStruct) {
  ...
  for (var field in userStruct) {
    if (userStruct.hasOwnProperty(field)) {
      var fieldInfo = userStruct.getFieldInfo(field);
      console.log('__serialize field', field);
      type = fieldInfo.getType();
      id = fieldInfo.getId();
      fields.push(this.writeFieldBegin(type, id));
      fields.push(this.serializeValue(userStruct[field], fieldInfo));
    }
  }
