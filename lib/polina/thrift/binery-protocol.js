...
polina.thrift.BineryProtocol.prototype.serializeValue = function(value, type) {
  var output;
  if(type === polina.thrift.Types.STRING) {
 ...
 * @param userStructure
 */
polina.thrift.BineryProtocol.prototype.serializeStruct = function(userStructure) {
  var fields = [], type, id;
  console.log('_serializeStruct');
  for (var field in  userStructure) {
    if (userStructure.hasOwnProperty(field)){
      console.log('__serialize field', field);
      type = userStructure.getFieldInfo(field).type;
      id = userStructure.getFieldInfo(field).id;
      fields.push(this.writeFieldBegin(type,id));
      fields.push(this.serializeValue(userStructure[field], type));
    }
  }
...
/**
 * @param {polina.thrift.TArray} array
 */
 ...
 * @inheritDoc
 */
polina.thrift.BineryProtocol.prototype.writeMap = function(ktype, vtype, size) {
  return Buffer.concat([this.writeByte(ktype),
                        this.writeByte(vtype),
                        this.writeI32(size)]);
};
