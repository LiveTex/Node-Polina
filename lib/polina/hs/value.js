...
polina.hs.Value.prototype.process = function(cursor, chunk) {
  ...
  while (valueCursor < chunk.length) {
    ...
    if (octet === polina.hs.SEPARATOR) {
      var data = chunk.slice(cursor.getPosition(), valueCursor).toString();
      if (!this.__header) {
        this.__header = data;
    ...
    if (octet === polina.hs.TERMINAL) {
      var response = chunk.slice(cursor.getPosition(), valueCursor);
      if (this.__length === -1) {
        this.__length = parseInt(response.toString(), 10);
      } else {
        this.__value = polina.hs.decodeResponse(response, this.__length);
      }
      cursor.incrPosition(valueCursor - cursor.getPosition() + 1);
