...
polina.Connection.prototype.__processData = function() {
  ...
  while (this.__requestHandlers.length > 0 && this.__readBuffer.length > 0) {
    ...
    var slicePosition = cursor.getPosition();

    if (cursor.getPosition() === 0) {

      cursor.incrPosition(handler.getHandledSize());

//      console.log('Position after reset cursor' + cursor.getPosition());
    }


    handler.process(cursor, this.__readBuffer);
  ...
  this.__readBuffer = this.__readBuffer.slice(slicePosition);
};
