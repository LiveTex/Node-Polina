...
polina.redis.Bucket.prototype.keys = function(pattern, complete, cancel) {
  function localContext(clients, pattern, complete, cancel) {
    var keys = [];
    var context = this;
    var tasksLeft = Object.keys(clients).length;

    for (var id in clients) {
      queryKeys.call(context, clients[id], pattern, localComplete, cancel);
    }

    function queryKeys(client, pattern, complete, cancel) {
      if (client !== undefined) {
    ...
    function localComplete(clientKeys) {
      keys = keys.concat(clientKeys);
      tasksLeft -= 1;
  ...
  localContext(this.__clients, pattern, complete, cancel);
};
...
polina.redis.Bucket.prototype.scriptLoad = function(lua, complete, cancel) {
  var client = this.__bucket[this.__getIndex(lua)];
  if (client !== undefined) {
    client.scriptLoad(lua, complete, cancel);
  } else {
    cancel('Redis bucket is incomplete.');
  }
};
...
polina.redis.Bucket.prototype.evalsha = function(sha, args, type,
                                                 complete, cancel) {
  var client = this.__bucket[this.__getIndex(sha)];
  if (client !== undefined) {
    client.evalsha(sha, args, type, complete, cancel);
  } else {
    cancel('Redis bucket is incomplete.');
  }
};
