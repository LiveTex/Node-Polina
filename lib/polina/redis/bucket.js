...
polina.redis.Bucket.prototype.__getIndex = function(key) {
  return util.hash.murmur(key, polina.__SEED) % this.__size;
};
 ...
 * @inheritDoc
 */
polina.redis.Bucket.prototype.exists = function(key, complete, cancel) {
  var client = this.__bucket[this.__getIndex(key)];
  if (client !== undefined) {
    client.exists(key, complete, cancel);
  } else {
    cancel('Redis bucket is incomplete.');
  }
};
 ...
 * @inheritDoc
 */
polina.redis.Bucket.prototype.persist =
 ...
 * @inheritDoc
 */
polina.redis.Bucket.prototype.scan = function(cursor, complete, cancel,
                                              opt_options) {

  var client = this.__bucket[this.__getIndex(cursor)];
  if (client !== undefined) {
 ...
 * @inheritDoc
 */
polina.redis.Bucket.prototype.sscan = function(key, cursor, complete, cancel,
                                              opt_options) {
 ...
 * @inheritDoc
 */
polina.redis.Bucket.prototype.hscan = function(key, cursor, complete, cancel,
                                              opt_options) {
