...
polina.redis.PacketHandler.prototype.process = function(cursor, chunk) {
  ...
  var subCursor =  cursor.getPosition();

  if (this.__value === null) {
    this.__value = polina.redis.resp.createValue(subCursor, chunk);
  }

  if (this.__value !== null) {
    subCursor = this.__value.process(subCursor, chunk);

    if (this.__value.isComplete()) {
      cursor.incrPosition(subCursor - cursor.getPosition());
