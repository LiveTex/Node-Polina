...
polina.redis.resp.BulkValue.prototype.process = function(cursor, chunk) {
 ...
 // console.log("Process BulkValue in " + cursor.getPosition());
  ...
  if (!this.__isComplete) {
    if ((valueCursor + this.__length + 1) < chunk.length) {
      ...
      this.__output = chunk.toString('utf8', valueCursor,
          valueCursor + this.__length);

      this.__handledSize += this.__length + 2;

      this.__isComplete = true;
//      console.log('is Complite ' + this.__output);
      valueCursor += this.__length + 2;
    }
    if (!this.__isComplete) {

//      console.log('********************* Bulk is uncomplite');
//      console.log('Bulk Length ' + this.__length);
//      console.log('Bulk __handledSize ', this.__handledSize);
//      console.log('Cursor ' + valueCursor);
    }
  }
