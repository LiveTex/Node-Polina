 ...
 * @param {!Buffer} chunk
 */
polina.redis.resp.__parseLength = function(header, cursor, chunk) {
  ...
  while (stop < chunk.length) {
    if ((chunk[stop] === polina.redis.resp.__TERMINAL) &&
      ...
      header.setLength(chunk.toString('utf8', start + 1, stop));
      break;
    }
    stop += 1;
  }
...
polina.redis.resp.HEADER_PARSER = function(header, cursor, chunk) {
  ...
  switch (status) {
    ...
    case polina.redis.resp.DataType.BULK: {
      ...
      polina.redis.resp.__parseLength(header, cursor, chunk);
      break;
    }
