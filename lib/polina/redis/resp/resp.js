...
polina.redis.resp.__parseNestedValue = function(value, cursor, chunk, length) {
  ...
  while (items.length !== length) {
    if ((items.length == 0) || (items[items.length - 1].isCompleted())) {
      items.push(new polina.redis.resp.Packet());
    }
    if (!polina.redis.resp.PACKET_PARSER(items[items.length - 1],
        cursor, chunk)) {
      break;
    }
  ...
  value.set(items);
  return ((items.length === length) && (items[length - 1].isCompleted()));
};
