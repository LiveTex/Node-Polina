...
polina.redis.Client.prototype.blpop =
  ...
  var args = ['BLPOP'].concat(keys).concat(timeout);
...
polina.redis.Client.prototype.brpop =
  ...
  var args = ['BRPOP'].concat(keys).concat(timeout);
  this.__arrCommand(args, complete, cancel);
};
...
polina.redis.Client.prototype.brpoplpush =
  ...
  var args = ['BRPOPLPUSH', source, destination, timeout];
  this.__arrCommand(args, complete, cancel);
};
...
polina.redis.Client.prototype.lindex = function(key, index, complete, cancel) {
  this.__strCommand(['LINDEX', key, index], complete, cancel);
};
...
polina.redis.Client.prototype.lrange =
  ...
  this.__arrCommand(['LRANGE', key, start, stop], complete, cancel);
};
...
polina.redis.Client.prototype.lrem =
  ...
  this.__arrCommand(['LREM', key, count, value], complete, cancel);
};
...
polina.redis.Client.prototype.lset =
  ...
  this.__strCommand(['LSET', key, index, value], complete, cancel);
};
...
polina.redis.Client.prototype.ltrim =
    function(key, start, stop, complete, cancel) {
  this.__strCommand(['LTRIM', key, start, stop], complete, cancel);
};
...
polina.redis.Client.prototype.rpoplpush =
  ...
  this.__strCommand(['LPOPLPUSH', source, destination], complete, cancel);
};
