...
polina.beans.User = function(tube) {
  polina.beans.Client.call(this, 'quit\r\n', 'use ' + tube.getName() + '\r\n',
      new polina.beans.PacketHandler('USING', polina.nop, polina.nop),
...
polina.beans.User.prototype.statsTube = function(complete) {
  ...
  this._command('stats-tube', this.__tube, polina.beans.Status.OK,
      function(jid, data) {
...
polina.beans.User.prototype.peekReady = function(complete) {
  this._command('peek-ready', '', polina.beans.Status.FOUND, complete,
      function(error) {
        complete('', '');
      });
};
 ...
 */
polina.beans.User.prototype.delete = function(jid, complete) {
