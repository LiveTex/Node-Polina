...
polina.beans.unsafeWatch = function(tube, handler, cancel) {
  ...
  function handle(jid, data) {
    watcher.delete(jid, watch, cancel);
    handler(data);
  }
 ...
 * @param {!function(string, number=)} cancel
 */
polina.beans.put = function(tube, data, complete, cancel) {
  if (typeof tube === 'string') {
    polina.beans.__putSerialized(polina.beans.reconstructTube(tube),
        tube, data, complete, cancel);
  } else {
...
/**
 * @param {!polina.beans.Tube} tube Beanstalkd tube.
 * @param {string} id Tube identifier.
 * @param {string} data Tube data.
 * @param {!function(string, string)} complete Result Handler.
 * @param {!function(string, number=)} cancel
 */
polina.beans.__putSerialized =
    function(tube, id, data, complete, cancel) {
  polina.beans.__selectUser(id, tube).put(0, 0, polina.beans.TASK_TTR,
      data, complete, cancel);
