...
polina.beans.PacketHandler.prototype.process = function(cursor, chunk) {
  var __cursor = cursor.getPosition();
  if (!this.__isHeadParsed) {
    ...
    while (i < chunk.length) {
      if (chunk[i] === 10) {
        ...
        this.__header = chunk.toString('utf8', __cursor, i - 1).split(' ');
  ...
  if (this.__isHeadParsed) {
    if (this.__header[0] === 'RESERVED' ||
      ...
      if (chunk.length >= __cursor + payloadLength + 2) {
        this.__isComplete = true;
        this.__body = chunk.slice(__cursor, __cursor + payloadLength).toString();

        __cursor += payloadLength + 2;
      }
    } else {
      this.__isComplete = true;
    }
  ...
  cursor.incrPosition(__cursor - cursor.getPosition());
};
