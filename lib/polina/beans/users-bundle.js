...
polina.beans.UsersBundle.prototype.put =
    function(priority, timeout, execTime, data, opt_complete, opt_cancel) {
      ...
      if (user !== undefined) {
        user.put(priority, timeout, execTime, data, opt_complete, opt_cancel);
      }
 ...
 */
polina.beans.UsersBundle.prototype.isRunning = function() {
  for (var i = 0, l = this.__users.length; i < l; i += 1) {
    if (this.__users[i].isRunning()) {
      return true;
    }
  }
